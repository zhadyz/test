...
try:
    ...
    enhancements_map = {
        ...
        'ac-2.6': 'Privilege management must be dynamic to adapt to changing roles and environments.',
        'ac-2.7': 'Privileged accounts are high-value targets and require strict role-based or attribute-based controls.',
        'ac-2.8': 'Dynamic account management allows for just-in-time access, reducing the window of exposure.',
        'ac-2.9': 'Shared accounts eliminate individual accountability and should be strictly limited.',
        'ac-2.10': 'Shared account credentials must be changed frequently to prevent former members from retaining access.',
        'ac-2.11': 'Usage conditions (e.g., time of day, location) add a layer of defense beyond simple authentication.',
        'ac-2.12': 'Atypical usage patterns often indicate compromise. Monitoring provides early detection.',
        'ac-2.13': 'High-risk individuals (e.g., terminated hostile employees) must have access revoked immediately.'
    }

    for control in data:
        cid = control['control_id'].lower()
        ...
        # Update AC-2
        if cid == 'ac-2':
            control['rationale'] = ac2_rationale
            
            if 'metadata' not in control: control['metadata'] = {}
            control['metadata']['stig_id'] = 'GEN000290'
            control['metadata']['cci'] = ['CCI-000015', 'CCI-000016', 'CCI-002145']
            
            # Merge/Ensure scripts
            if 'implementation_scripts' not in control: control['implementation_scripts'] = {}
            if 'windows' not in control['implementation_scripts'] or 'Not applicable' in str(control['implementation_scripts'].get('windows', '')):
                control['implementation_scripts']['windows'] = ac2_scripts['windows']
            # Preserve existing linux scripts if they are good, otherwise update? 
            # The existing linux scripts were from CAC and seemed good (auditd). I won't overwrite them if they exist.

            # Update internal enhancements list rationale
            for enh in control.get('enhancements', []):
                eid = enh.get('id').lower()
                if eid in enhancements_map:
                    enh['rationale'] = enhancements_map[eid]

        # Update Top-Level Enhancements (ac-2.1, ac-2.2, etc.)
        if cid in enhancements_map:
            control['rationale'] = enhancements_map[cid]
            # Add scripts to enhancements too, so they appear populated
            if 'implementation_scripts' not in control: control['implementation_scripts'] = {}
            
            # Use the same scripts as AC-2 for now as a base, since they are relevant
            if 'windows' not in control['implementation_scripts']:
                control['implementation_scripts']['windows'] = ac2_scripts['windows']
            if 'linux' not in control['implementation_scripts']:
                control['implementation_scripts']['linux'] = ac2_scripts['linux']

    with open('backend/data/controls_catalog.json', 'w', encoding='utf-8') as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
        print("Successfully updated AC-1, AC-2, and AC-2 enhancements.")
