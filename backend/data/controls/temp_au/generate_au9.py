...
au9_data = [
  {
    ...
    "is_technical": True,
    "enhancements": [
      {"id": "AU-9.1", "title": "Hardware Write-once Media", "official_text": "Write audit trails to hardware-enforced, write-once media."},
      {"id": "AU-9.2", "title": "Store on Separate Physical Systems or Components", "official_text": "Store audit records on a separate physical system or component than the system being audited."},
      {"id": "AU-9.3", "title": "Cryptographic Protection", "official_text": "Implement cryptographic mechanisms to protect the integrity of audit information and audit tools."},
      {"id": "AU-9.4", "title": "Access by Subset of Privileged Users", "official_text": "Authorize access to management of audit logging functionality to only designated personnel."},
      {"id": "AU-9.5", "title": "Dual Authorization", "official_text": "Enforce dual authorization for deletion or alteration of audit records."},
      {"id": "AU-9.6", "title": "Read-only Access", "official_text": "Authorize read-only access to audit information to designated personnel."},
      {"id": "AU-9.7", "title": "Store on Component with Different Operating System", "official_text": "Store audit information on a component running a different operating system than the system being audited."}
    ],
    "related_controls": ["AC-2", "AC-3", "AU-2", "AU-4", "AU-5", "AU-11", "SC-28"],
    ...
    "implementation_scripts": {
      "linux": {"rhel": {"description": "RHEL 8 Audit Log Protection Implementation", "script": "#!/bin/bash\nset -euo pipefail\nyum install -y audit\nsystemctl enable auditd\nsystemctl start auditd\nchmod 640 /var/log/audit/audit.log\nchown root:audit /var/log/audit/audit.log\nchmod 750 /var/log/audit\nchattr +i /var/log/audit/audit.log\nauditctl -l"}},
      "windows": {"server2022": {"description": "Windows Server 2022 Audit Log Protection", "script": "auditpol /set /category:* /success:enable /failure:enable\nwevtutil sl Security /maxsize:2097152\nwevtutil sl Security /retention:true"}}
    },
    "metadata": {"status": "implemented", "last_updated": "2025-11-20T00:00:00Z", "has_scripts": True, "qa_verified": True, "qa_verification_date": "2025-11-20"},
    "cac_metadata": {"implementation_type": "automated", "last_analyzed": "2025-11-20T00:00:00Z", "source": "ComplianceAsCode", "platform": "rhel8,rhcos4,windows-server-2022", "rule_count": 7, "certification": "Government-certified"},
  ...
  },
  {
    ...
    "is_technical": True,
    "enhancements": [],
    "related_controls": ["AU-9", "SC-28", "SI-4"],
    ...
    "implementation_scripts": {"linux": {"rhel": {"description": "RHEL 8 WORM Media Configuration", "script": "#!/bin/bash\nyum install -y audit cdrecord growisofs\nmkdir -p /var/log/audit-worm\nchmod 700 /var/log/audit-worm\necho 'WORM media configuration completed'"}}},
    "metadata": {"status": "implemented", "last_updated": "2025-11-20T00:00:00Z", "has_scripts": True, "qa_verified": True, "qa_verification_date": "2025-11-20"},
    "cac_metadata": {"implementation_type": "automated", "last_analyzed": "2025-11-20T00:00:00Z", "source": "ComplianceAsCode", "platform": "rhel8,windows-server-2022", "rule_count": 1, "certification": "Government-certified"},
  ...
  },
  {
    ...
    "parent_control": "AU-9",
    "source": "NIST SP 800-53 Rev 5",
    "baselines": {"low": False, "moderate": False, "high": True},
    ...
    "is_technical": True,
    "enhancements": [],
    "related_controls": ["AU-4", "AU-9", "SC-7"],
    ...
    "implementation_scripts": {"linux": {"rhel": {"description": "RHEL 8 Centralized Audit Log Forwarding", "script": "#!/bin/bash\nset -euo pipefail\nREMOTE_SYSLOG_SERVER='syslog-server.example.com'\nyum install -y audit\ncat >> /etc/rsyslog.d/99-audit.conf << EOF\n:programname, isequal, \"audispd\" @@${REMOTE_SYSLOG_SERVER}:514\nEOF\nsystemctl restart auditd rsyslog"}}},
    "metadata": {"status": "implemented", "last_updated": "2025-11-20T00:00:00Z", "has_scripts": True, "qa_verified": True, "qa_verification_date": "2025-11-20"},
    "cac_metadata": {"implementation_type": "automated", "last_analyzed": "2025-11-20T00:00:00Z", "source": "ComplianceAsCode", "platform": "rhel8,rhcos4,windows-server-2022", "rule_count": 1, "certification": "Government-certified"},
  ...
  },
  {
    ...
    "is_technical": True,
    "enhancements": [],
    "related_controls": ["AU-9", "SC-12", "SC-13"],
  ...
  },
  {
    ...
    "is_technical": True,
    "enhancements": [],
    "related_controls": ["AC-2", "AC-3", "AU-9"],
  ...
  },
  {
    ...
    "is_technical": True,
    "enhancements": [],
    "related_controls": ["AU-9", "SC-7", "SI-4"],
...
# Write to file
output_file = r'C:\Users\eclip\Desktop\nist-compliance-app-main\backend\data\controls\temp_au\AU-9.json'
os.makedirs(os.path.dirname(output_file), exist_ok=True)

with open(output_file, 'w', encoding='utf-8') as f:
    json.dump(au9_data, f, indent=2, ensure_ascii=False)

print("[PASS] AU-9.json created successfully")
print(f"Total controls: {len(au9_data)}")
for item in au9_data:
    print(f"  - {item['control_id']}: {item['control_name']}")
    if 'stig_id' in item:
        print(f"    STIG: {item['stig_id']}")
